...
#: What the phrase "the decision session" means for a release whose execution
#: boundary is NOT the session its information came from. A release that
#: declares nothing gets 0 - the decision session IS the information session,
#: which is what every registration before R62.3.3 was held to - so this
#: widens nothing that did not ask to be widened.
ENTRY_SESSION_DECLARATION = "the ENTRY session"
NEXT_OPEN_BOUNDARY = "NEXT_ELIGIBLE_SESSION_OPEN"


def execution_offset_sessions(registration: dict) -> dict:
    """How far the DECISION session sits from the registrar's first eligible one.

    The registrar resolves a first eligible OBSERVATION session from the
    calendar and that answer is immutable. But a release may execute on a LATER
    boundary than the session it read: ``REVERSED_SPY_PUT_CALL_SKEW_H5_NEXT_OPEN_V1``
    forms its rule from session t's 15:45 ET snapshot and enters at the OPEN of
    t+1, so the session it freezes a decision FOR - the session its emission
    window, its entry mark and its horizon are all measured on - is t+1.

    Counting that decision on session t would be wrong in three places at once:
    the window would open the day before the information existed, the freeze
    would be looked up under a key the research owner never wrote, and the
    horizon would end one session early. So the offset is ASKED of the release's
    ...
    if not declared:
        return {"offset_sessions": 0, "declared": False,
                ...
                "reason": "the release declares no execution contract, so the "
                          "decision session is the information session"}
    decision_is = str(declared.get("decision_session_is") or "")
    boundary = str(declared.get("execution_boundary") or "")
    if decision_is == ENTRY_SESSION_DECLARATION and boundary == NEXT_OPEN_BOUNDARY:
        return {"offset_sessions": 1, "declared": True,
                "decision_session_is": ENTRY_SESSION_DECLARATION,
                "execution_boundary": boundary,
                "reason": "the release declares that it enters at the next "
                          "eligible session's open, so its decision session is "
                          "one eligible session after the information session"}
    return {"offset_sessions": 0, "declared": True,
            "decision_session_is": decision_is or "the INFORMATION session",
            "execution_boundary": boundary or None,
            "reason": "the release declares an execution contract that does "
                      "not move the decision session"}
...
def _shift_eligible(session: str, n: int) -> Optional[str]:
    """``session`` advanced by ``n`` ELIGIBLE sessions on the exchange calendar.

    The calendar is the estate's one session authority and it knows sessions
    that have not printed yet, which is the whole point here: the entry session
    of a decision frozen tonight is tomorrow, and tomorrow is not in any
    realised price panel. Returns ``None`` only if the calendar cannot answer,
    and the caller then keeps the unshifted session rather than inventing one.
    """
    ...
    try:
        ...
        return None
    try:
        cur = date.fromisoformat(str(d))
    except (TypeError, ValueError):
        return None
    steps = 0
    guard = 0
    while steps < int(n) and guard < 400:
        guard += 1
        ...
        if not _EC.is_non_session(cur):
            steps += 1
    return cur.isoformat() if steps == int(n) else None


def decision_grid(registration: dict, *, sessions: list,
                  cadence_sessions: Optional[int] = None,
                  offset_sessions: int = 0) -> list:
    ...
    ``offset_sessions`` moves the WHOLE grid later by that many eligible
    sessions, for a release whose declared execution boundary is a later session
    than the one its information came from (see
    :func:`execution_offset_sessions`). It is 0 for every release that declares
    nothing, so the grid such a release gets is byte-identical to the one it got
    ...
    try:
        shift = max(0, int(offset_sessions or 0))
    except (TypeError, ValueError):
        shift = 0
    if shift and first:
        # Step along the EXCHANGE CALENDAR, never by calendar days and never
        # along the realised panel. Calendar days would put a Thanksgiving entry
        # on a day the exchange is shut; the realised panel cannot answer at all
        # while the entry session is still in the future, and a challenger
        # waiting for tomorrow is ARMED, not finished.
        shifted = _shift_eligible(first, shift)
        if shifted is None:
            # The calendar could not answer. Keeping the registrar's session is
            # the STRICTER outcome - an earlier window, judged sooner - so an
            # unavailable calendar can never widen anything.
            shift = 0
        else:
            first = shifted
    if first:
...
def assess_registration(*, registration: dict, series: dict,
    ...
    # R62.3.3 - the decision session is the release's own, not this module's
    # assumption. Asked once, reported on the row, and 0 for every release that
    # declares nothing.
    exec_offset = execution_offset_sessions(reg)
    out["execution_offset"] = dict(exec_offset)
