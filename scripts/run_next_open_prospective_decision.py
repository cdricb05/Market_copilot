...
def advance(args) -> int:
    """The operator fallback, and the runtime's own path, in ONE command.

    It calls exactly what ``alpha_agent.r52.runtime`` calls, so a session
    advanced by hand and a session advanced by the schedule are the same act
    recorded the same way. Nothing here decides anything: every rule lives in
    the owner below.
    """
    from alpha_agent.alpha_recovery import next_open_runtime as NOR

    res = NOR.advance_daily(now=args.now,
                            information_session=args.session,
                            execute_append=not args.no_append)
    if args.json:
        print(json.dumps(res, indent=1, sort_keys=True, default=str))
        return 0
    print("challenger          : %s" % res.get("challenger_id"))
    print("information session : %s" % res.get("information_session"))
    print("entry session       : %s" % res.get("entry_session"))
    print("first legal entry   : %s" % res.get("first_legal_entry_session"))
    pub = res.get("publication") or {}
    if pub:
        print("publication         : %s (available=%s)"
              % (pub.get("outcome"), pub.get("available")))
    rep = res.get("publication_report") or {}
    if rep:
        print("first available     : %s" % rep.get("FIRST_AVAILABLE_TIME"))
        print("publication delay   : %s" % rep.get("PUBLICATION_DELAY"))
        print("before next open    : %s" % rep.get("AVAILABLE_BEFORE_NEXT_OPEN"))
    app = res.get("append") or {}
    if app:
        print("append              : %s  priced $%s  spent $%s"
              % (app.get("state"), app.get("total_priced_usd"),
                 app.get("spent_estimate_usd", 0.0)))
    print("entry state         : %s" % res.get("entry_state"))
    fr = res.get("freeze") or {}
    if fr:
        print("freeze              : %s (%s)" % (fr.get("frozen"),
                                                 fr.get("outcome")))
    if res.get("detail"):
        print("detail              : %s" % res["detail"])
    print("paid dollars        : $0.00")
    print(res.get("state"))
    return 0 if res.get("state") != NOR.ADV_BLOCKED else 2
...
def main() -> int:
    ...
    ap.add_argument("--advance", action="store_true",
                    help="THE operator fallback: one command that derives the "
                         "information session from the clock, probes "
                         "publication, appends the session to the owned "
                         "surface and freezes the decision if the window is "
                         "open. This is the SAME owner the research runtime "
                         "calls, so running it by hand can never disagree "
                         "with what the runtime would have done.")
    ap.add_argument("--no-append", action="store_true",
                    help="--advance: price the append but never download")
